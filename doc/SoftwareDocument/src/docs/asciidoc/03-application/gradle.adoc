...
include::{adrsdir}/building/gradle-or-maven.adoc[leveloffset=+1]

include::{adrsdir}/building/gradle-kotlin-dsl.adoc[leveloffset=+1]

== Usage

include::{adrsdir}/building/dependencies_declaration.adoc[leveloffset=+2]
...
Due to the easy creation of extending the Gradle build with custom plugins, we simply need to declare our dependencies in the build files; nothing else left to do.
External (non-project) dependencies are declared with the custom code to centralize all declarations along with (consistent) version numbers.
