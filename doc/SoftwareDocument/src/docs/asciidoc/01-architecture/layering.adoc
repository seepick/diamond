...
* `app` - the main entry point to start up the application; has a dependency on all Koin submodules (incl. impls) and wires the dependency injection context; reads the configuration and starts the Ktor webserver.
* `view` - container for all HTTP API related routes and controllers
** `view-routing` - the endpoint definition and reading of relevant data from the HTTP request; exception/failure handling
** [line-through]#`view-model`# - No! as those DTOs are (OpenAPI) generated and oustourced
...
** `controller-impl` - request/response mapping; transformation from view-models (OpenAPI spec) into domain-models (clean business entities)
* `domain`` - container for business logic/rules, entities/value objects (free from any framework/tech-references!); semi-hexagonal architecture
...
(YAML, WSDL)
* `shared` - container for common functionality/infrastructure code (can be potentially outsourced over time)
...
** `shared-xxx` - ... more cross-cutting concerns (metrics, HTTP, scheduling, security)...
...
* `api-SDK` - containing the OpenAPI specification (Yaml) and generated (Kotlin Multiplatform) sources (see chapter <<development_codegeneration>>)
** The OpenAPI spec is a contract and owned by developers (backend and frontend), testers, and business alike; our service is "just an implementation" of it, tied to an interface (=contract).
** `api-model` - entities supported by specific serialization framework (usable by `client-sdk`)
** `api-server-routes` - webframework specific server side code to tie the contract to the implementation (jvm only)
...
include::{adrsdir}/application/layer-granularity.adoc[leveloffset=+1]
