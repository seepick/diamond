...
The following looks at the aspect of test target, the actual scope what's being tested.
Unit tests for single classes/functions; integration tests for the whole application without externals; end2end for the while system landscape; (no tests for single modules necessary).
...
** Use the Ktor test-engine to improve speed; pretending to do HTTP without the need of actually communicating it (port allocating, etc.).
...
.Sample Integration Test using Cucumber
[source,gherkin]
----
...
* Details:
** Using Karate (Cucumber?) test framework, only change base URL to target another environment.
...
.End-to-end system tests written using Karate
