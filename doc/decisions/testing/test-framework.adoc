...
* JUnit
** Obviously, legacy; mature, well-known.
* TestNG
** The more modern follow-up of JUnit.
** Focuses also on integration tests (opposed to JUnit's focus on, well... units).
* Spek
** Nice, but not really.
* Kotest
** A test framework and assertion library (property testing)
** A Kotlin-idiomatic framework using code over config (speak: annotations).
** Can reuse JUnit 5 engine or run with its own (Gradle integration).
** BAD: IDE integration a bit worse (dynamic nature of added tests).
* Cucumber
** For BDD, not regular tests...
