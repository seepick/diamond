...
* Faster Development
** Concise, expressive (less boilerplate, less verbose); cleaner/less code,
* Higher stability, fewer bugs
** Null safety built into the type system
** Immutability by default (collections)
* High performance with coroutine support
* Functional (functions as first-class citizens; higher-order functions)
* Extension functions (discoverability, auto-completion)
* Great Tooling: JetBrains supported (IDE!), growing ecosystem

Side points for Kotlin:

* Concise lambda definition
* Easy to write custom DSLs
* Type inference, smart casts
* Simple delegates with `by`
* Bypass type erasure with inline functions and reified generics
** Meaning, having the type parameter available at runtime, no more `typeOf` magic
